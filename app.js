...
function searchItems(searchValue, filter) {
  ...
  if (filter === "category") {
    searchResult = items.filter((item) =>
      simpleString(item.category).includes(simpleString(searchValue))
    );
    updateList(searchResult);
  } else {
    searchResult = items.filter((item) =>
      simpleString(item.name).includes(simpleString(searchValue))
    );
    updateList(searchResult);
  }
...
function updateList(arr) {
  ...
  arr.sort(sortItemsAscending).forEach((item) => {
    ...
    //   searchItems(this.innerText, "category");
    // });
...
// TODO:
// create actual tracking window [DONE]
// update overlay window in realtime [DONE]
// (?)create item pages []
// (?)add a collected button to track collected items []

// 1. get item database [DONE]
// 2. create new array from database ( itemDatabase = itemArray) [DONE]
// 3. filter the new array for item categories [HALF DONE]
// 4. allow independant searching within those categories []

// BUGS:
// Resets list on removing from tracker.
